...
public class Hand extends ArrayList<Card>{
    public static final int NORTH = 2, EAST = 3, SOUTH = 0, WEST = 1;
    ...
    public void paint(Graphics g)
    {
        ...
        if (showPoints && this.size() == 0)
        {
            ...
            int x = (position == NORTH || position == SOUTH ?
                screenSize.width / 2 - 60 : (position == WEST ? 5 : screenSize.width - 130));
    ...
    public void setToPlay(int index)
    {
        this.get(index).setToPlay(true);
        Card c = this.get(index);
        switch (position) {
            case Hand.SOUTH: // intentional fall-through
            case Hand.NORTH:
                c.setLocation(screenSize.width / 2 - (c.getWidth() / 2),
                        screenSize.height / 2 - c.getHeight() - (c.getWidth() / 2) - 12
                        + (position == Hand.NORTH ? 0 : c.getHeight() + c.getWidth()));
                break;
            case Hand.WEST: // intentional fall-through
            case Hand.EAST:
                c.setLocation(screenSize.width / 2 - c.getHeight() - (c.getWidth() / 2)
                        + (position == Hand.WEST ? 0 : c.getHeight() + c.getWidth()),
